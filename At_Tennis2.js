...
// draw the line parallel to the vector with size the length of the bat
...
function onMouseMove(evt){
    ...
    // REDESIGN CODE: ALTERNATE APPROACH: TREAT THE BAT AS A PARTICLE.
    /*
       BALL AND SWIPE COLLISION
       if( gesureStart)
       {
        ballSwipe(var Pos), var swipePos)
        {
            var swipedir =   ballPos.subtract(swipePos);
         if(swipeDir.lengthSquared() < ball.radius*ball.radius){
            // use this vector to calculate the force mv on the ball
         }
          // use the scaled screen radius 
...
function testBallBounce(obj)
{
	...
	for (var i=0; (i<bouncingSurfaces.length && hasHitAWall==false); i++){
		...
		if (( (dist.length() < obj.radius) || (testTunneling) ) &&  test){
            ...
            // need to fire an event to give the wall position the ball is on.
		}
